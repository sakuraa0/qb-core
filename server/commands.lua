...
QBCore.Commands.Add('tp', Lang:t("command.tp.help"), { { name = Lang:t("command.tp.params.x.name"), help = Lang:t("command.tp.params.x.help") }, { name = Lang:t("command.tp.params.y.name"), help = Lang:t("command.tp.params.y.help") }, { name = Lang:t("command.tp.params.z.name"), help = Lang:t("command.tp.params.z.help") } }, false, function(source, args)
    if args[1] and not args[2] and not args[3] then
        ...
        if target ~= 0 then
            ...
            TriggerClientEvent('okokNotify:Alert', source, 'Sistema', Lang:t('error.not_online'), 5000, 'error')
        end
    else
            ...
            if location then
                ...
                TriggerClientEvent('okokNotify:Alert', source, 'Sistema', Lang:t('error.location_not_exist'), 5000, 'error')
            end
        end
    else
        if args[1] and args[2] and args[3] then
            ...
            if x ~= 0 and y ~= 0 and z ~= 0 then
                ...
                TriggerClientEvent('okokNotify:Alert', source, 'Sistema', Lang:t('error.wrong_format'), 5000, 'error')
            end
        else
            TriggerClientEvent('okokNotify:Alert', source, 'Sistema', Lang:t('error.missing_args'), 5000, 'error')
        end
    end
end, 'admin')
...
QBCore.Commands.Add('openserver', Lang:t("command.openserver.help"), {}, false, function(source)
    if not QBCore.Config.Server.Closed then
        TriggerClientEvent('okokNotify:Alert', source, 'Sistema', Lang:t('error.server_already_open'), 5000, 'error')
        return
    end
    if QBCore.Functions.HasPermission(source, 'admin') then
        QBCore.Config.Server.Closed = false
        TriggerClientEvent('okokNotify:Alert', source, 'Sistema', Lang:t('success.server_opened'), 5000, 'success')
    else
...
QBCore.Commands.Add('closeserver', Lang:t("command.closeserver.help"), {{ name = Lang:t("command.closeserver.params.reason.name"), help = Lang:t("command.closeserver.params.reason.help")}}, false, function(source, args)
    if QBCore.Config.Server.Closed then
        TriggerClientEvent('okokNotify:Alert', source, 'Sistema', Lang:t('error.server_already_closed'), 5000, 'error')
        return
    end
    if QBCore.Functions.HasPermission(source, 'admin') then
        ...
        TriggerClientEvent('okokNotify:Alert', source, 'Sistema', Lang:t('success.server_closed'), 5000, 'success')
    else
        QBCore.Functions.Kick(source, Lang:t("error.no_permission"), nil, nil)
    end
end, 'admin')
...
QBCore.Commands.Add('me', Lang:t("command.me.help"), {{name = Lang:t("command.me.params.message.name"), help = Lang:t("command.me.params.message.help")}}, false, function(source, args)
    if #args < 1 then TriggerClientEvent('okokNotify:Alert', source, 'Sistema', Lang:t('error.missing_args2'), 5000, 'error') return end
