...
RegisterNetEvent('QBCore:ToggleDuty', function()
    ...
    if Player.PlayerData.job.onduty then
        ...
        TriggerClientEvent('okokNotify:Alert', source, 'Sistema', Lang:t('info.off_duty'), 5000, 'info')
    else
        Player.Functions.SetJobDuty(true)
        TriggerClientEvent('okokNotify:Alert', source, 'Sistema', Lang:t('info.on_duty'), 5000, 'info')
    end
...
RegisterNetEvent('QBCore:CallCommand', function(command, args)
    ...
    if hasPerm then
        if QBCore.Commands.List[command].argsrequired and #QBCore.Commands.List[command].arguments ~= 0 and not args[#QBCore.Commands.List[command].arguments] then
            TriggerClientEvent('okokNotify:Alert', source, 'Sistema', Lang:t('error.missing_args2'), 5000, 'error')
        else
            QBCore.Commands.List[command].callback(src, args)
        end
    else
        TriggerClientEvent('okokNotify:Alert', source, 'Sistema', Lang:t('error.no_access'), 5000, 'error')
    end
end)
